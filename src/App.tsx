...
export const AuthContext = React.createContext<{email: string | null; login: ()=>void; logout: ()=>void}>({
...
function Sidebar() {
  ...
  const { logout, email } = React.useContext(AuthContext);
  ...
  const links = [
    ...
    { to: '/meals', label: 'Meals (Master)', icon: UtensilsCrossed },
    { to: '/combos', label: 'Vendor Combos', icon: Package },
    { to: '/admins', label: 'Account Permissions', icon: Users },
    { to: '/logs', label: 'Unified Logs', icon: Activity },
  ...
  return (
    <aside className="w-64 bg-slate-900 flex flex-col border-r border-slate-800">
      <div className="p-6 border-b border-slate-800 flex items-center gap-3 shrink-0">
        ...
        <span className="font-bold text-slate-100 tracking-tight text-xl">ALLOWANCE <span className="text-indigo-400">PRO</span></span>
      </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="h-16 bg-white border-b border-slate-200 px-8 flex items-center justify-between shadow-sm z-10 shrink-0">
          <div className="flex items-center gap-8">
            <h1 className="text-lg font-semibold text-slate-800">CEO Control Panel</h1>
            <div className="flex gap-4 items-center text-xs font-medium">
              <span className="bg-slate-100 px-2 py-1 rounded text-slate-600">Uptime: 99.9%</span>
...
function AppRouter() {
  ...
  if (!email) {
    return <AuthContext.Provider value={{ email, login, logout }}><Login /></AuthContext.Provider>;
  }
