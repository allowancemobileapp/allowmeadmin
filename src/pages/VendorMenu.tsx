  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Link to="/vendors" className="p-2 border border-slate-200 dark:border-slate-800 rounded-md hover:bg-slate-50 dark:hover:bg-slate-800/50 text-slate-600 dark:text-slate-400 transition-colors">
          <ArrowLeft className="w-4 h-4" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-800 dark:text-slate-200">Menu: {vendor?.name}</h1>
      ...
      <div className="flex flex-wrap gap-2 border-b border-slate-200 dark:border-slate-800 pb-2">
         {SECTIONS.map(s => (
            <button 
               ...
               className={`px-4 py-2 rounded-t-lg text-sm font-bold transition-colors ${activeSection === s.id ? 'bg-indigo-50 text-indigo-700 border-b-2 border-indigo-600' : 'text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-slate-800'}`}
            >
               {s.name}
            </button>
         ))}
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm p-6 max-w-3xl">
        ...
        <form onSubmit={handleAddOrEdit} className="flex gap-4 items-end">
          <div className="flex flex-col gap-1 flex-1">
            <label className="font-semibold text-slate-600 dark:text-slate-400 text-sm">Select Master Meal</label>
            <select value={selectedMealId} onChange={e=>setSelectedMealId(e.target.value)} className="border border-slate-200 dark:border-slate-800 rounded px-3 py-2 w-full focus:outline-none focus:ring-1 focus:ring-indigo-500">
                <option value="">-- Choose Meal --</option>
                {sectionMeals.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
            </select>
          </div>
          <div className="flex flex-col gap-1 w-32">
            <label className="font-semibold text-slate-600 dark:text-slate-400 text-sm">{activeSection === 1 ? 'Portion' : 'Quantity'}</label>
            {activeSection === 1 ? (
              <select value={quantityPortion} onChange={e=>setQuantityPortion(e.target.value)} required className="border border-slate-200 dark:border-slate-800 rounded px-3 py-2 w-full focus:outline-none focus:ring-1 focus:ring-indigo-500">
          ...
          <div className="flex flex-col gap-1 w-32">
            <label className="font-semibold text-slate-600 dark:text-slate-400 text-sm">Price (₦)</label>
            <input 
          ...
          <tbody className="divide-y divide-slate-100">
            {sectionItems.map(item => (
              <tr key={item.id} className="hover:bg-slate-50/50">
                <td className="px-6 py-4 text-slate-800 dark:text-slate-200 font-medium">{item.meal_name}</td>
                <td className="px-6 py-4 text-slate-600 dark:text-slate-400">{item.quantity_portion}</td>
