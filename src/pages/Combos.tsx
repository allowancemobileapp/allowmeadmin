...
export default function Combos() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white dark:bg-slate-900 p-6 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm overflow-x-auto">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-800 dark:text-slate-200">Vendor Combos</h1>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 flex-col-reverse relative">
        ...
        <div className={`bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm p-6 h-[max-content] xl:sticky xl:top-6 ${editingComboId ? 'border-2 border-indigo-500 ring-4 ring-indigo-50' : ''}`}>
            <div className="flex justify-between items-center mb-4 border-b border-slate-100 pb-2">
                <h2 className="text-sm font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wider">{editingComboId ? 'Edit Custom Combo' : 'Create Custom Combo'}</h2>
              ...
              <form onSubmit={handleManualSubmit} className="space-y-6">
                <div className="space-y-3">
                  {manualRows.map((row, index) => {
                      ...
                      return (
                      <div key={index} className="flex flex-col gap-2 p-3 border border-slate-200 dark:border-slate-800 rounded bg-slate-50 dark:bg-slate-950 relative">
                        <div className="flex gap-2 w-full">
                          <select 
                            ...
                            className="flex-1 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs rounded px-2 py-2 focus:ring-indigo-500 focus:border-indigo-500"
                          >
                          ...
                          {isMain ? (
                             <select 
                                ...
                                className="w-24 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs rounded px-2 py-2 focus:ring-indigo-500"
                             >
                            ...
                            <input 
                               ...
                               className="w-16 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs rounded px-2 py-2 focus:ring-indigo-500"
                            />
                          )}
                        </div>
                        <select 
                            ...
                            className="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs rounded px-2 py-2 focus:ring-indigo-500 focus:border-indigo-500 disabled:opacity-50"
                  ...
                  <button type="button" onClick={() => setManualRows([...manualRows, {section: '', itemId: null, quantity: 1, portion: 'Full'}])} className="w-full py-2 border-2 border-dashed border-slate-300 dark:border-slate-700 text-slate-500 text-xs font-bold rounded hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:text-indigo-600 transition-colors uppercase tracking-wider">
                    + Add Row
                  </button>
                </div>

                <div className="border-t border-slate-100 pt-4">
                  <label className="font-semibold text-slate-600 dark:text-slate-400 text-xs mb-1 block uppercase tracking-wider">Food Group</label>
                  {foodGroups.length > 0 ? (
                      <select
                         value={groupId} onChange={e=>setGroupId(e.target.value)}
                         className="border border-slate-300 dark:border-slate-700 rounded px-3 py-2 text-sm w-full focus:outline-none focus:ring-1 focus:ring-indigo-500"
                      >
        ...
        <div className="xl:col-span-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm p-6 overflow-x-auto h-[max-content]">
          <div className="max-w-md mb-6 flex items-center gap-4 border-b border-slate-100 pb-6">
            <div className="w-full">
                <label className="font-semibold text-slate-600 dark:text-slate-400 text-sm mb-2 block">Choose Vendor to manage</label>
                <select 
                value={vendorId} 
                onChange={(e) => setVendorId(e.target.value)}
                className="w-full border border-slate-200 dark:border-slate-800 rounded-lg px-4 py-2 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-shadow"
                >
          ...
          {vendorId ? (
            <div>
              <h2 className="text-sm font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wider mb-4">Saved Combos ({options.length})</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {options.map((opt) => {
                  const groupName = getGroupName(opt.group_id);
                  return (
                  <div key={opt.id} className="p-4 border border-slate-200 dark:border-slate-800 rounded-lg hover:border-slate-300 transition-colors bg-slate-50 dark:bg-slate-950 flex flex-col relative group">
                    ...
                    <h3 className="font-bold text-slate-800 dark:text-slate-200 text-sm mb-2">{opt.combo_description}</h3>
                    <div className="flex-1 text-xs text-slate-500">
                       <ul className="list-disc pl-4 space-y-0.5">
                         {(typeof opt.items === 'string' ? JSON.parse(opt.items) : (opt.items || [])).map((item: any, idx: number) => (
                           <li key={idx}>{item.name} {item.portion ? `(${item.portion})` : ''} <span className="font-bold text-slate-700 dark:text-slate-300">{item.quantity && item.quantity > 1 ? `x${item.quantity}` : ''}</span></li>
                         ))}
                       </ul>
                    </div>
                    <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-200 dark:border-slate-800">
                      <div>
                        ...
                        <span className="font-bold text-slate-900 dark:text-slate-100">₦{Number(opt.total_price).toLocaleString()}</span>
                      </div>
                ...
                {options.length === 0 && (
                  <div className="col-span-full py-8 text-center text-slate-500 font-medium bg-slate-50 dark:bg-slate-950 rounded-lg border border-dashed border-slate-200 dark:border-slate-800">
      ...
      {showAutoForm && (
        <div className="fixed inset-0 bg-slate-900/50 flex flex-col z-50">
           <div className="flex-1 overflow-y-auto p-4 md:p-8">
             <div className="max-w-4xl mx-auto bg-white dark:bg-slate-900 rounded-xl shadow-xl overflow-hidden flex flex-col max-h-full">
               <div className="p-6 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950">
                 <div>
                   <h2 className="text-xl font-bold text-slate-800 dark:text-slate-200">Auto-Generated Combos</h2>
               ...
               <div className="p-6 overflow-y-auto bg-slate-100 dark:bg-slate-800/50 flex-1">
                  {autoCombosList.length === 0 ? (
                      <div className="text-center py-12 text-slate-500 font-medium bg-white dark:bg-slate-900 rounded-lg border border-dashed border-slate-300 dark:border-slate-700">
                          Could not generate any valid combos from this vendor's menu based on constraints.
                      </div>
                  ) : (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {autoCombosList.map((c, i) => (
                        <label key={i} className={`flex items-start gap-3 p-4 rounded-lg border-2 cursor-pointer transition-colors ${c.selected ? 'bg-indigo-50 border-indigo-500' : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 hover:border-slate-300'}`}>
                            ...
                            <h4 className="font-bold text-slate-800 dark:text-slate-200 text-sm leading-tight mb-2">{c.description}</h4>
               ...
               <div className="p-6 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex justify-between items-center shrink-0">
                 <span className="text-sm font-bold text-slate-600 dark:text-slate-400">{autoCombosList.filter(c => c.selected).length} selected</span>
                 <div className="flex gap-3">
                   <button onClick={() => setShowAutoForm(false)} className="px-4 py-2 font-bold text-sm text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800/50 hover:bg-slate-200 rounded-lg">Cancel</button>
